...
class PublicOitivaController extends Controller
{
    ...
    public function assistir(Request $request, Oitiva $oitiva)
    {
        ...
        // 5.Gerar URL da Transcrição
        $transcriptionUrl = null;
        if (!empty($oitiva->transcricao)) {
            $transcriptionUrl = URL::temporarySignedRoute(
                'public.oitiva.transcricao',
                now()->addHours(2),
                ['oitiva' => $oitiva->id]
            );
        }
    ...
    /**
     *Retorna a transcrição da oitiva em formato SRT
     */
    public function transcricao(Request $request, Oitiva $oitiva)
    {
        // Verifica se a oitiva possui transcrição
        if (empty($oitiva->transcricao)) {
            return response()->json(['message' => 'Transcrição não disponível'], 404);
        }

        // Retorna a transcrição como texto
        return response($oitiva->transcricao, 200)
            ->header('Content-Type', 'text/plain; charset=utf-8');
    }

    /**
     * Inicia o processamento do download ZIP
     */
